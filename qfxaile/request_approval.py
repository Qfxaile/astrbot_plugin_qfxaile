...
class RequestApprovalService:
    ...
    def __init__(self, client: Any, store: Any, settings: PluginSettings) -> None:
        self.client = client
        self.store = store
    ...
    async def handle_request(self, raw: Mapping[str, Any]) -> None:
        ...
        if self.settings.value(
            "agree_auto_approve_admin_request", True
        ) and self.settings.is_admin("agree_admin_user_ids", user_id):
            try:
                await self._approve(request_info, True)
            except Exception as exc:
                logger.warning(f"自动审批申请失败: {exc}")
            return
        notify_group_id = str(self.settings.value("agree_notify_group_id", "")).strip()
        if not notify_group_id:
            return
        admins = self.settings.admins("agree_admin_user_ids")
        text = (
    ...
    async def decide(self, reply_id: str, decision: str, sender_id: str) -> str:
        ...
        if not self.settings.is_admin("agree_admin_user_ids", sender_id):
